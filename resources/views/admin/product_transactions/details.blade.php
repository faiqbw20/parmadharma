    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white flex flex-col gap-y-5 overflow-hidden p-10 shadow-sm sm:rounded-lg">
                    ...
                    <div class="flex flex-col gap-y-5 col-span-2">
                        ...
                        @forelse($productTransaction->transactionDetails as $detail)
                            <div class="item-card flex flex-row justify-between items-center">
                                <div class="flex-col flex md:flex-row items-center gap-x-3">
                                    <img src="{{ Storage::url($detail->product->photo) }}" alt="{{ $detail->product->name }}" class="w-[50px] h-[50px]">
                                    <div>
                                        <h3 class="text-2xl font-bold text-indigo-950">
                                            {{ $detail->product->name }}
                                        </h3>
                ...
                @role('owner')
                @if($productTransaction->is_paid)
                    <a href="https://wa.me/" class="w-fit font-bold py-3 px-5 rounded-full text-white bg-indigo-700">
                        WhastApp Customer
                    </a>
                @else
                ...
                @role('buyer')
                <a href="https://wa.me/081281290895" class="w-fit font-bold py-3 px-5 rounded-full text-white bg-indigo-700">
                    Contact Admin
                </a>
                @endrole
